...
module.exports = {
	...
	async execute(interaction) {
		if (interaction.member.permissions.has(PermissionsBitField.Flags.Administrator)) {
			...
			await interaction.client.channels.cache.get('1061406583478833223').send(`:warning: \`${interaction.member.nickname}\` (\`${interaction.member.user.username}\`) reset the value of the \`${fixedName}\` counter to \`${newValue}\`.`)
		}
		else {
